...
// import { cashfreeResponse } from "../types/types";
...
export class WhatsAppService {
  ...
  static formatRegistrationResponse(
    ...
    const paymentLink = createPaymentLink(customerDetails, 100);
    const payment_url = `https://www.msudcacyberia.in/chatbot?fullName=${fullName}&email=${email}&contactNo=${contactNo}&contactNo2=${contactNo2}&institute=${institute}&year=${year}&level=${level}&age=${age}&gender=${gender}&stream=${stream}&events=${events}`;
...
Team Registration: https://whatsform.com/aplTii`;
      ...
      if (soloData) {
        try {
          ...
          const paymentLink = await createPaymentLink(customerDetails, 100);

          await this.sendMessage(
            from,
            `🎉 Registration Successful!\nTo complete your payment, click the link below:\n${paymentLink}\n\nAfter payment, you will receive your ticket URL.`
          );
        } catch (error) {
          await this.sendMessage(
            from,
